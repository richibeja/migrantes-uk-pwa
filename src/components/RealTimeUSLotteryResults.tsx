...
export default function RealTimeUSLotteryResults() {
  const {
    results,
    lastUpdate,
    isLive,
  ...
  const getStatusText = (source: string) => {
    return source === 'real' ? 'Datos Reales' : 'Datos Simulados';
  };
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl p-6 text-white">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-2xl font-bold">Loterías de Estados Unidos</h2>
              <p className="text-blue-100">Datos en tiempo real</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white bg-opacity-10 rounded-lg p-3">
            ...
            <div className="text-blue-100 text-sm">Loterías</div>
          </div>
          <div className="bg-white bg-opacity-10 rounded-lg p-3">
            <div className="text-2xl font-bold">{stats.dataAccuracy}%</div>
            <div className="text-blue-100 text-sm">Precisión</div>
          </div>
          <div className="bg-white bg-opacity-10 rounded-lg p-3">
            <div className="text-2xl font-bold">{stats.totalWinners}</div>
            <div className="text-blue-100 text-sm">Ganadores</div>
          </div>
          <div className="bg-white bg-opacity-10 rounded-lg p-3">
        ...
        <div className="mt-4 flex items-center justify-between text-sm text-blue-100">
          <div className="flex items-center space-x-2">
            ...
            <span>
              Última actualización: {formatTime(lastUpdate)} ({updateCount} actualizaciones)
            </span>
          </div>
          <div className="flex items-center space-x-2">
            ...
            {highestJackpots.slice(0, 4).map((lottery) => (
              <div key={lottery.id} className="bg-gradient-to-r from-green-50 to-blue-50 rounded-lg p-4 border border-green-200">
                ...
                <div className="text-sm text-gray-600">
                  Próximo sorteo: {formatDate(lottery.nextDraw)} a las {formatTime(lottery.nextDraw)}
                </div>
              </div>
            ))}
          </div>
            ...
            <div key={lottery.id} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <div className="text-sm text-gray-600">
                    {lottery.winners.jackpot} ganadores
                  </div>
                </div>
              </div>
            </div>
      ...
      {frequentNumbers.length > 0 && (
        <div className="bg-white rounded-2xl shadow-lg p-6">
          <h3 className="text-xl font-bold text-gray-900 mb-4 flex items-center space-x-2">
            ...
            <span>Números Más Frecuentes</span>
          </h3>
