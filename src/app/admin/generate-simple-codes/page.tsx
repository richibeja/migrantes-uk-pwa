...
export default function AdminGenerateSimpleCodesPage() {
  ...
  const loadCodigos = () => {
    const todosCodigos = codigosSimples.obtenerCodigos();
    setCodigos(todosCodigos);
    const usados = todosCodigos.filter(c => c.usado).length;
    const disponibles = todosCodigos.filter(c => !c.usado).length;
    setMessage(`Total: ${todosCodigos.length} | Usados: ${usados} | Disponibles: ${disponibles}`);
  };
  ...
  const generarCodigo = () => {
    ...
    try {
      const codigo = codigosSimples.generarCodigo(userEmail, plan);
      
      setUserEmail('');
      setMessage(`Código generado: ${codigo}`);
      setError('');
      loadCodigos();
  ...
  const eliminarCodigo = (codigo: string) => {
    if (confirm('¿Estás seguro de que quieres eliminar este código?')) {
      const eliminado = codigosSimples.eliminarCodigo(codigo);
      if (eliminado) {
  ...
  const getStatusColor = (usado: boolean, expira: string) => {
    if (usado) return 'text-green-400 bg-green-900/30';
    if (new Date() > new Date(expira)) return 'text-red-400 bg-red-900/30';
    return 'text-blue-400 bg-blue-900/30';
  };

  const getStatusText = (usado: boolean, expira: string) => {
    if (usado) return 'Usado';
    if (new Date() > new Date(expira)) return 'Expirado';
    return 'Disponible';
  };
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-3xl font-bold text-white mb-2">
                🔑 Generar Códigos Únicos Simples
              </h1>
              <p className="text-gray-300">
                Genera códigos únicos que SÍ funcionan para activar cuentas
              </p>
            </div>
            <a
              href="/admin"
                  ...
                  {codigos.map((codigo, index) => (
                    <tr key={index} className="border-b border-gray-600/50">
                      ...
                      <td className="py-3 px-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(codigo.usado, codigo.expira)}`}>
                          {getStatusText(codigo.usado, codigo.expira)}
                        </span>
                      </td>
                      <td className="py-3 px-4 text-gray-300">{formatDate(codigo.creado)}</td>
                      <td className="py-3 px-4 text-gray-300">{formatDate(codigo.expira)}</td>
                      <td className="py-3 px-4">
                        <button
